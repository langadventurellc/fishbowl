...
describe('Feature: AI Provider Configuration', () => {
  ...
  describe('Scenario: Add OpenAI API key', () => {
    it('should save API key securely', async () => {
      ...
      await $('[data-testid="provider-select"]').selectByAttribute('value', 'openai');
      ...
      await expect($('[data-testid="provider-openai-status"]')).toHaveText('Active');
    });
  });
  ...
  describe('Scenario: Test AI provider connection', () => {
    it('should verify API key works', async () => {
      ...
      await expect($('[data-testid="connection-status"]')).toHaveText('Connected');
    });
  });
});
```
...
module.exports = {
  ...
  apps: {
    'ios.debug': {
      type: 'ios.app',
      binaryPath: 'apps/mobile/ios/build/Build/Products/Debug-iphonesimulator/FishbowlAI.app',
      build: 'cd apps/mobile && xcodebuild -workspace ios/FishbowlAI.xcworkspace -scheme FishbowlAI -configuration Debug -sdk iphonesimulator -derivedDataPath ios/build'
    },
    'android.debug': {
      type: 'android.apk',
...
describe('Feature: AI Provider Configuration', () => {
  ...
  describe('Scenario: Add OpenAI API key', () => {
    it('should save API key securely', async () => {
      ...
      await expect(element(by.text('Active').withAncestor(by.id('provider-openai')))).toBeVisible();
    });
  });
...
**packages/shared/src/services/ai/__tests__/ai-service.test.ts**
```typescript
...
describe('AIService', () => {
  ...
  describe('addProvider', () => {
    it('should save API key and initialize provider', async () => {
      ...
      // Then
      expect(mockSecureStorage.saveAPIKey).toHaveBeenCalledWith(provider, apiKey);
    ...
    it('should throw error for unknown provider', async () => {
      ...
      // When/Then
      await expect(aiService.addProvider(provider, apiKey))
